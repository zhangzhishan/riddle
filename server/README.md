...
- `POST /api/device/messages` with the device capability in the `Authorization` bearer header, plus `Content-Type: image/png`, `X-Device-Id`, and a raw PNG body no larger than 4 MiB.
